...
HttpEngine.prototype.step = function step(requestSpec, ee) {
  ...
  var f = function(context, callback) {
    ...
    request(requestParams, function requestCallback(err, res, body) {
      ...
      // Single capture retrocompatibility
      if (params.capture) {
        params.capture.variables = params.capture.variables || [];
        ['json', 'xpath', 'regexp', 'header'].forEach(function(type) {
          if(params.capture.hasOwnProperty(type)) {
            params.capture.type = type;
            params.capture.variables.push({
              expression: params.capture[type],
              transform: params.capture.transform,
              as: params.capture.as,
              group: params.capture.group
            });
            delete params.capture[type];
            delete params.capture.transform;
            delete params.capture.as;
            delete params.capture.group;
          }
        })
      }
      ...
      if (params.capture || params.match) {
        let parser;
        let extractor;
        if ((params.capture && params.capture.type === 'json') || (params.match && params.match.json)) {
          parser = parseJSON;
          extractor = extractJSONPath;
        } else if (xmlCapture && (params.capture && params.capture.type === 'xpath') || (params.match && params.match.xpath)) {
          parser = xmlCapture.parseXML;
          extractor = xmlCapture.extractXPath;
        } else if ((params.capture && params.capture.type === 'regexp') || (params.match && params.match.regexp)) {
          parser = dummyParser;
          extractor = extractRegExp;
        } else if ((params.capture && params.capture.type === 'header') || (params.match && params.match.header)) {
          parser = dummyParser;
        ...
        if ((params.capture && params.capture.type === 'header') ||
        ...
        parser(content, function(err2, doc) {
          ...
          if (params.capture) {
            params.capture.variables.forEach(function(variable) {
              let result;
              let expr = variable.expression;
              if (params.capture.type === "regexp" && variable.group) {
                result = extractor(doc, expr, variable.group);
              } else {
                result = extractor(doc, expr);
              }
              context.vars[variable.as] = result;
              debug('capture: %s = %s', variable.as, result);

              if (variable.transform) {
                let result2 = engineUtil.evil(
                  context.vars,
                  variable.transform);
                context.vars[variable.as] = result2;
                debug('transform: %s = %s', variable.as, context.vars[variable.as]);
              }
            });
          }
